...
package object jsonic {
  ...
  implicit class JsResultCanDoThings(private val j: JsResult) extends AnyVal {
    ...
    def dbls: Option[Array[Double]] = j match { case jad: JsArrD => Some(jad.doubles); case _ => None }
  ...
  implicit class OptionalJsResultCanDoThings(private val oj: Option[JsResult]) extends AnyVal {
    ...
    def dbls: Option[Array[Double]] = oj match { case Some(jad: JsArrD) => Some(jad.doubles); case _ => None }
