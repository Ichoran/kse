...
package packed {
  final class ByteAsBox(val B: Byte) extends AnyVal {
    ...
    @inline def bit0To(b: Boolean) = new ByteAsBox((if (b) (B | 0x01) else (B & 0xFE)).toByte)
    @inline def bit1To(b: Boolean) = new ByteAsBox((if (b) (B | 0x02) else (B & 0xFD)).toByte)
    @inline def bit2To(b: Boolean) = new ByteAsBox((if (b) (B | 0x04) else (B & 0xFB)).toByte)
    @inline def bit3To(b: Boolean) = new ByteAsBox((if (b) (B | 0x08) else (B & 0xF7)).toByte)
    @inline def bit4To(b: Boolean) = new ByteAsBox((if (b) (B | 0x10) else (B & 0xEF)).toByte)
    @inline def bit5To(b: Boolean) = new ByteAsBox((if (b) (B | 0x20) else (B & 0xDF)).toByte)
    @inline def bit6To(b: Boolean) = new ByteAsBox((if (b) (B | 0x40) else (B & 0xBF)).toByte)
    @inline def bit7To(b: Boolean) = new ByteAsBox((if (b) (B | 0x80) else (B & 0x7F)).toByte)
    @inline def bitTo(i: Int)(b: Boolean) = new ByteAsBox((if (b) B | (1<<i) else B & (0xFF - (1<<i))).toByte)
  ...
  final class ShortAsBox(val S: Short) extends AnyVal {    
    ...
    @inline def bit0To(b: Boolean)  = new ShortAsBox((if (b) (S | 0x0001) else (S & 0xFFFE)).toShort)
    @inline def bit1To(b: Boolean)  = new ShortAsBox((if (b) (S | 0x0002) else (S & 0xFFFD)).toShort)
    @inline def bit2To(b: Boolean)  = new ShortAsBox((if (b) (S | 0x0004) else (S & 0xFFFB)).toShort)
    @inline def bit3To(b: Boolean)  = new ShortAsBox((if (b) (S | 0x0008) else (S & 0xFFF7)).toShort)
    @inline def bit4To(b: Boolean)  = new ShortAsBox((if (b) (S | 0x0010) else (S & 0xFFEF)).toShort)
    @inline def bit5To(b: Boolean)  = new ShortAsBox((if (b) (S | 0x0020) else (S & 0xFFDF)).toShort)
    @inline def bit6To(b: Boolean)  = new ShortAsBox((if (b) (S | 0x0040) else (S & 0xFFBF)).toShort)
    @inline def bit7To(b: Boolean)  = new ShortAsBox((if (b) (S | 0x0080) else (S & 0xFF7F)).toShort)
    @inline def bit8To(b: Boolean)  = new ShortAsBox((if (b) (S | 0x0100) else (S & 0xFEFF)).toShort)
    @inline def bit9To(b: Boolean)  = new ShortAsBox((if (b) (S | 0x0200) else (S & 0xFDFF)).toShort)
    @inline def bit10To(b: Boolean) = new ShortAsBox((if (b) (S | 0x0400) else (S & 0xFBFF)).toShort)
    @inline def bit11To(b: Boolean) = new ShortAsBox((if (b) (S | 0x0800) else (S & 0xF7FF)).toShort)
    @inline def bit12To(b: Boolean) = new ShortAsBox((if (b) (S | 0x1000) else (S & 0xEFFF)).toShort)
    @inline def bit13To(b: Boolean) = new ShortAsBox((if (b) (S | 0x2000) else (S & 0xDFFF)).toShort)
    @inline def bit14To(b: Boolean) = new ShortAsBox((if (b) (S | 0x4000) else (S & 0xBFFF)).toShort)
    @inline def bit15To(b: Boolean) = new ShortAsBox((if (b) (S | 0x8000) else (S & 0x7FFF)).toShort)
    @inline def bitTo(i: Int)(b: Boolean) = new ShortAsBox((if (b) S | (1<<i) else S & (0xFFFF - (1<<i))).toShort)
  ...
  final class IntAsBox(val I: Int) extends AnyVal {
    ...
    @inline def bit0To(b: Boolean)  = new IntAsBox(if (b) (I | 0x00000001) else (I & 0xFFFFFFFE))
    @inline def bit1To(b: Boolean)  = new IntAsBox(if (b) (I | 0x00000002) else (I & 0xFFFFFFFD))
    @inline def bit2To(b: Boolean)  = new IntAsBox(if (b) (I | 0x00000004) else (I & 0xFFFFFFFB))
    @inline def bit3To(b: Boolean)  = new IntAsBox(if (b) (I | 0x00000008) else (I & 0xFFFFFFF7))
    @inline def bit4To(b: Boolean)  = new IntAsBox(if (b) (I | 0x00000010) else (I & 0xFFFFFFEF))
    @inline def bit5To(b: Boolean)  = new IntAsBox(if (b) (I | 0x00000020) else (I & 0xFFFFFFDF))
    @inline def bit6To(b: Boolean)  = new IntAsBox(if (b) (I | 0x00000040) else (I & 0xFFFFFFBF))
    @inline def bit7To(b: Boolean)  = new IntAsBox(if (b) (I | 0x00000080) else (I & 0xFFFFFF7F))
    @inline def bit8To(b: Boolean)  = new IntAsBox(if (b) (I | 0x00000100) else (I & 0xFFFFFEFF))
    @inline def bit9To(b: Boolean)  = new IntAsBox(if (b) (I | 0x00000200) else (I & 0xFFFFFDFF))
    @inline def bit10To(b: Boolean) = new IntAsBox(if (b) (I | 0x00000400) else (I & 0xFFFFFBFF))
    @inline def bit11To(b: Boolean) = new IntAsBox(if (b) (I | 0x00000800) else (I & 0xFFFFF7FF))
    @inline def bit12To(b: Boolean) = new IntAsBox(if (b) (I | 0x00001000) else (I & 0xFFFFEFFF))
    @inline def bit13To(b: Boolean) = new IntAsBox(if (b) (I | 0x00002000) else (I & 0xFFFFDFFF))
    @inline def bit14To(b: Boolean) = new IntAsBox(if (b) (I | 0x00004000) else (I & 0xFFFFBFFF))
    @inline def bit15To(b: Boolean) = new IntAsBox(if (b) (I | 0x00008000) else (I & 0xFFFF7FFF))
    @inline def bit16To(b: Boolean) = new IntAsBox(if (b) (I | 0x00010000) else (I & 0xFFFEFFFF))
    @inline def bit17To(b: Boolean) = new IntAsBox(if (b) (I | 0x00020000) else (I & 0xFFFDFFFF))
    @inline def bit18To(b: Boolean) = new IntAsBox(if (b) (I | 0x00040000) else (I & 0xFFFBFFFF))
    @inline def bit19To(b: Boolean) = new IntAsBox(if (b) (I | 0x00080000) else (I & 0xFFF7FFFF))
    @inline def bit20To(b: Boolean) = new IntAsBox(if (b) (I | 0x00100000) else (I & 0xFFEFFFFF))
    @inline def bit21To(b: Boolean) = new IntAsBox(if (b) (I | 0x00200000) else (I & 0xFFDFFFFF))
    @inline def bit22To(b: Boolean) = new IntAsBox(if (b) (I | 0x00400000) else (I & 0xFFBFFFFF))
    @inline def bit23To(b: Boolean) = new IntAsBox(if (b) (I | 0x00800000) else (I & 0xFF7FFFFF))
    @inline def bit24To(b: Boolean) = new IntAsBox(if (b) (I | 0x01000000) else (I & 0xFEFFFFFF))
    @inline def bit25To(b: Boolean) = new IntAsBox(if (b) (I | 0x02000000) else (I & 0xFDFFFFFF))
    @inline def bit26To(b: Boolean) = new IntAsBox(if (b) (I | 0x04000000) else (I & 0xFBFFFFFF))
    @inline def bit27To(b: Boolean) = new IntAsBox(if (b) (I | 0x08000000) else (I & 0xF7FFFFFF))
    @inline def bit28To(b: Boolean) = new IntAsBox(if (b) (I | 0x10000000) else (I & 0xEFFFFFFF))
    @inline def bit29To(b: Boolean) = new IntAsBox(if (b) (I | 0x20000000) else (I & 0xDFFFFFFF))
    @inline def bit30To(b: Boolean) = new IntAsBox(if (b) (I | 0x40000000) else (I & 0xBFFFFFFF))
    @inline def bit31To(b: Boolean) = new IntAsBox(if (b) (I | 0x80000000) else (I & 0x7FFFFFFF))
    @inline def bitTo(i: Int)(b: Boolean) = new IntAsBox(if (b) I | (1<<i) else I & (0xFFFFFFFF - (1<<i)))
  ...
  final class LongAsBox(val L: Long) extends AnyVal {
    ...
    @inline def bit0To(b: Boolean)  = new LongAsBox(if (b) (L | 0x0000000000000001L) else (L & 0xFFFFFFFFFFFFFFFEL))
    @inline def bit1To(b: Boolean)  = new LongAsBox(if (b) (L | 0x0000000000000002L) else (L & 0xFFFFFFFFFFFFFFFDL))
    @inline def bit2To(b: Boolean)  = new LongAsBox(if (b) (L | 0x0000000000000004L) else (L & 0xFFFFFFFFFFFFFFFBL))
    @inline def bit3To(b: Boolean)  = new LongAsBox(if (b) (L | 0x0000000000000008L) else (L & 0xFFFFFFFFFFFFFFF7L))
    @inline def bit4To(b: Boolean)  = new LongAsBox(if (b) (L | 0x0000000000000010L) else (L & 0xFFFFFFFFFFFFFFEFL))
    @inline def bit5To(b: Boolean)  = new LongAsBox(if (b) (L | 0x0000000000000020L) else (L & 0xFFFFFFFFFFFFFFDFL))
    @inline def bit6To(b: Boolean)  = new LongAsBox(if (b) (L | 0x0000000000000040L) else (L & 0xFFFFFFFFFFFFFFBFL))
    @inline def bit7To(b: Boolean)  = new LongAsBox(if (b) (L | 0x0000000000000080L) else (L & 0xFFFFFFFFFFFFFF7FL))
    @inline def bit8To(b: Boolean)  = new LongAsBox(if (b) (L | 0x0000000000000100L) else (L & 0xFFFFFFFFFFFFFEFFL))
    @inline def bit9To(b: Boolean)  = new LongAsBox(if (b) (L | 0x0000000000000200L) else (L & 0xFFFFFFFFFFFFFDFFL))
    @inline def bit10To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000000400L) else (L & 0xFFFFFFFFFFFFFBFFL))
    @inline def bit11To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000000800L) else (L & 0xFFFFFFFFFFFFF7FFL))
    @inline def bit12To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000001000L) else (L & 0xFFFFFFFFFFFFEFFFL))
    @inline def bit13To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000002000L) else (L & 0xFFFFFFFFFFFFDFFFL))
    @inline def bit14To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000004000L) else (L & 0xFFFFFFFFFFFFBFFFL))
    @inline def bit15To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000008000L) else (L & 0xFFFFFFFFFFFF7FFFL))
    @inline def bit16To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000010000L) else (L & 0xFFFFFFFFFFFEFFFFL))
    @inline def bit17To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000020000L) else (L & 0xFFFFFFFFFFFDFFFFL))
    @inline def bit18To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000040000L) else (L & 0xFFFFFFFFFFFBFFFFL))
    @inline def bit19To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000080000L) else (L & 0xFFFFFFFFFFF7FFFFL))
    @inline def bit20To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000100000L) else (L & 0xFFFFFFFFFFEFFFFFL))
    @inline def bit21To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000200000L) else (L & 0xFFFFFFFFFFDFFFFFL))
    @inline def bit22To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000400000L) else (L & 0xFFFFFFFFFFBFFFFFL))
    @inline def bit23To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000000800000L) else (L & 0xFFFFFFFFFF7FFFFFL))
    @inline def bit24To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000001000000L) else (L & 0xFFFFFFFFFEFFFFFFL))
    @inline def bit25To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000002000000L) else (L & 0xFFFFFFFFFDFFFFFFL))
    @inline def bit26To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000004000000L) else (L & 0xFFFFFFFFFBFFFFFFL))
    @inline def bit27To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000008000000L) else (L & 0xFFFFFFFFF7FFFFFFL))
    @inline def bit28To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000010000000L) else (L & 0xFFFFFFFFEFFFFFFFL))
    @inline def bit29To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000020000000L) else (L & 0xFFFFFFFFDFFFFFFFL))
    @inline def bit30To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000040000000L) else (L & 0xFFFFFFFFBFFFFFFFL))
    @inline def bit31To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000080000000L) else (L & 0xFFFFFFFF7FFFFFFFL))
    @inline def bit32To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000100000000L) else (L & 0xFFFFFFFEFFFFFFFFL))
    @inline def bit33To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000200000000L) else (L & 0xFFFFFFFDFFFFFFFFL))
    @inline def bit34To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000400000000L) else (L & 0xFFFFFFFBFFFFFFFFL))
    @inline def bit35To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000000800000000L) else (L & 0xFFFFFFF7FFFFFFFFL))
    @inline def bit36To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000001000000000L) else (L & 0xFFFFFFEFFFFFFFFFL))
    @inline def bit37To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000002000000000L) else (L & 0xFFFFFFDFFFFFFFFFL))
    @inline def bit38To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000004000000000L) else (L & 0xFFFFFFBFFFFFFFFFL))
    @inline def bit39To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000008000000000L) else (L & 0xFFFFFF7FFFFFFFFFL))
    @inline def bit40To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000010000000000L) else (L & 0xFFFFFEFFFFFFFFFFL))
    @inline def bit41To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000020000000000L) else (L & 0xFFFFFDFFFFFFFFFFL))
    @inline def bit42To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000040000000000L) else (L & 0xFFFFFBFFFFFFFFFFL))
    @inline def bit43To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000080000000000L) else (L & 0xFFFFF7FFFFFFFFFFL))
    @inline def bit44To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000100000000000L) else (L & 0xFFFFEFFFFFFFFFFFL))
    @inline def bit45To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000200000000000L) else (L & 0xFFFFDFFFFFFFFFFFL))
    @inline def bit46To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000400000000000L) else (L & 0xFFFFBFFFFFFFFFFFL))
    @inline def bit47To(b: Boolean) = new LongAsBox(if (b) (L | 0x0000800000000000L) else (L & 0xFFFF7FFFFFFFFFFFL))
    @inline def bit48To(b: Boolean) = new LongAsBox(if (b) (L | 0x0001000000000000L) else (L & 0xFFFEFFFFFFFFFFFFL))
    @inline def bit49To(b: Boolean) = new LongAsBox(if (b) (L | 0x0002000000000000L) else (L & 0xFFFDFFFFFFFFFFFFL))
    @inline def bit50To(b: Boolean) = new LongAsBox(if (b) (L | 0x0004000000000000L) else (L & 0xFFFBFFFFFFFFFFFFL))
    @inline def bit51To(b: Boolean) = new LongAsBox(if (b) (L | 0x0008000000000000L) else (L & 0xFFF7FFFFFFFFFFFFL))
    @inline def bit52To(b: Boolean) = new LongAsBox(if (b) (L | 0x0010000000000000L) else (L & 0xFFEFFFFFFFFFFFFFL))
    @inline def bit53To(b: Boolean) = new LongAsBox(if (b) (L | 0x0020000000000000L) else (L & 0xFFDFFFFFFFFFFFFFL))
    @inline def bit54To(b: Boolean) = new LongAsBox(if (b) (L | 0x0040000000000000L) else (L & 0xFFBFFFFFFFFFFFFFL))
    @inline def bit55To(b: Boolean) = new LongAsBox(if (b) (L | 0x0080000000000000L) else (L & 0xFF7FFFFFFFFFFFFFL))
    @inline def bit56To(b: Boolean) = new LongAsBox(if (b) (L | 0x0100000000000000L) else (L & 0xFEFFFFFFFFFFFFFFL))
    @inline def bit57To(b: Boolean) = new LongAsBox(if (b) (L | 0x0200000000000000L) else (L & 0xFDFFFFFFFFFFFFFFL))
    @inline def bit58To(b: Boolean) = new LongAsBox(if (b) (L | 0x0400000000000000L) else (L & 0xFBFFFFFFFFFFFFFFL))
    @inline def bit59To(b: Boolean) = new LongAsBox(if (b) (L | 0x0800000000000000L) else (L & 0xF7FFFFFFFFFFFFFFL))
    @inline def bit60To(b: Boolean) = new LongAsBox(if (b) (L | 0x1000000000000000L) else (L & 0xEFFFFFFFFFFFFFFFL))
    @inline def bit61To(b: Boolean) = new LongAsBox(if (b) (L | 0x2000000000000000L) else (L & 0xDFFFFFFFFFFFFFFFL))
    @inline def bit62To(b: Boolean) = new LongAsBox(if (b) (L | 0x4000000000000000L) else (L & 0xBFFFFFFFFFFFFFFFL))
    @inline def bit63To(b: Boolean) = new LongAsBox(if (b) (L | 0x8000000000000000L) else (L & 0x7FFFFFFFFFFFFFFFL))
    @inline def bitTo(i: Int)(b: Boolean) = new LongAsBox(if (b) L | (1L<<i) else L & (-1L - (1L<<i)))
